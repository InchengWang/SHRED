...
# =============================================================================
#  请将此代码块完整地复制到您的 models.py 文件中，替换旧的 SDN 类
# =============================================================================
class SDN(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size, l1=350, l2=400, dropout=0.0):
        """
        初始化函数。
        重要提示：这里的 input_size 必须是 (序列长度 * 传感器数量)
        """
    ...
    def forward(self, x):
        """
        前向传播函数。
        x 的输入形状是 (批次大小, 序列长度, 传感器数量)
        """
        # 关键步骤：将输入张量展平，同时保留批次维度。
        # 形状变化: (批次大小, 序列长度, 传感器数量) -> (批次大小, 序列长度 * 传感器数量)
        x_flattened = x.view(x.size(0), -1)
        
        # 现在，x_flattened 的形状与 self.linear1 的输入维度完全匹配
        output = self.linear1(x_flattened)
        output = self.dropout(output)
        output = torch.nn.functional.relu(output)
