...
def partition_data_seq_mul(load_X, n, lags):
    ...
    interval10 = int(0.10*n) # 10% of n
    train_indices = np.arange(0, 7*interval10) # train indices are first 60% of n
    valid_indices = np.arange(7*interval10 + 1, 9*interval10 - int(lags/2)) # valid indices are next 20% of n
    test_indices = np.arange(9*interval10 - int(lags/2) + 1, n-lags) # test indices are last 20% of n
...
def transform_data_mul(load_X, load_XT, train_indices):
  """
    Normalize input data using MinMaxScaler based on training set statistics
  ...
  sc = sc.fit(load_X[train_indices])
  transformed_X = sc.transform(load_X)
  transformed_XT = sc.transform(load_XT)

  return transformed_X,transformed_XT, sc
